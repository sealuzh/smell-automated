...
def getRandoopCall(seed, configId, config, project, clazz, id, strategy, coreIndex):
  ...
  # for root, dirnames, filenames in os.walk(JTEXPERT_LIBS):
  #   for filename in fnmatch.filter(filenames, '*.jar'):
  #       class_path.append(os.path.join(root, filename))
  ...
  result += JAVA + " " + JTEXPERT_JAR
  result += " -cp " +class_path
  to_look_for = clazz.split('.')[-1] + '.java'
  source_path = find(pattern=to_look_for, path=SOURCE_PROJECTS+project)
  result += " -jf "+ source_path # source file
  result += " -maxTime 60"
  result += " -tp " + TESTDIR + "/s" + str(seed) + "/" + project
...
def createJobs(minSeed, maxSeed, configId, config, startNum, strategy="randoop.main.Main gentests"):
  ...
  path_1 = "%s/%s_JTExpert_%d.sh" %(SCRIPTDIR, USERNAME, JOB_ID)
  ...
  for seed in range(minSeed, maxSeed):
    ...
    for entry in CLASSES:
      ...
      if num >= ENTRIES_PER_JOB:
        ...
        path_2 = "%s/%s_JTExpert_%d.sh" %(SCRIPTDIR, USERNAME, JOB_ID)
...
print "Total number of calls to JTExpert: %d" % CALL_ID
